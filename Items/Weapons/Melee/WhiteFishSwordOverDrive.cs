...
{
	public class WhiteFishSwordOverDrive : ModItem
	{
		...
		public override bool Shoot(Player player, Terraria.DataStructures.EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockBack)
		{
			for (int i = -1; i <= 1; i++)
			{
				Vector2 shotVelocity = velocity.RotatedBy(MathHelper.ToRadians(i * 14f));
				Projectile.NewProjectile(source, position, shotVelocity, type, (int)(damage * 0.70f), knockBack, player.whoAmI, 9f);
			}
			return false;
		}
		...
		{
			CreateRecipe()
				.AddIngredient(ModContent.ItemType<global::VinesMod.Items.Weapons.Melee.WhiteFishSword>())
