...
namespace VinesMod.Projectiles
{
	...
	public class LaserBeamBlue : ModProjectile
	{
		...
		public override void AI()
		{
			...
			else
			{
				...
				for (int k = 0; k < chargeFact + 1; k++)
				{
					...
					Dust dust = Main.dust[Dust.NewDust(pos, 20, 20, DustID.Electric, Projectile.velocity.X / 2f,
			...
			for (int i = 0; i < 2; ++i)
			{
				...
				Dust dust = Main.dust[Dust.NewDust(dustPos, 0, 0, DustID.Electric, dustVel.X, dustVel.Y, 0, new Color(), 1f)];
				dust.noGravity = true;
				dust.scale = 1.2f;
			...
			if (Main.rand.Next(5) == 0)
			{
				Vector2 offset = Projectile.velocity.RotatedBy(1.57f, new Vector2()) * ((float)Main.rand.NextDouble() - 0.5f) *
								 Projectile.width;
				Dust dust = Main.dust[
					Dust.NewDust(dustPos + offset - Vector2.One * 4f, 8, 8, DustID.Smoke, 0.0f, 0.0f, 100, new Color(), 1.5f)];
				...
				unit.Normalize();
				dust = Main.dust[
					Dust.NewDust(Main.player[Projectile.owner].Center + 55 * unit, 8, 8, DustID.Smoke, 0.0f, 0.0f, 100, new Color(), 1.5f)];
				dust.velocity = dust.velocity * 0.5f;
				dust.velocity.Y = -Math.Abs(dust.velocity.Y);
			}
			#endregion
